...
└── .github/workflows/daily_update.yml   # 自動排程 (每 3 天)
```
...
完成後,**之後每 3 天(每月 1、4、7…31 日)台灣時間早上 8:00 自動更新**,
不用再手動做任何事。要臨時更新就回 **Actions → Daily Update → Run workflow**
手動觸發。
...
自動排程會在爬完後歸檔新文章並推進 vault repo,本機的
...
描述由 `digest.py` 每次更新依該議題近期文章自動產生(`digest.json` 的
  ...
  "items": [
    {
      ...
      "first_seen_at": "2026-06-16T08:00:00+08:00 (第一次被爬到的時間,BaseCrawler 自動維護)",
      "summary": "摘要,200 字內",
      "tags": ["fuel", "biofuel"],
      "topic_ids": ["bio_fuel"],
...
workflow 的最後一步 (`scripts/check_crawler_health.py`) 會在**部署完成後**
檢查所有來源:任一來源失敗就讓該次 run 變紅 (GitHub 會寄通知信),並在 Actions
run summary 顯示各來源狀態表,爬蟲掛掉當次執行就會被發現。

`first_seen_at` 記錄每篇文章第一次被爬到的時間;72 小時內新收錄的文章,前端
會標綠色 **NEW** 徽章,天天看的人一眼就知道哪些是今天新進的。
...
涵蓋 BRIEF 排序規則、時間窗、議題關鍵字比對、NEW 判定、BaseCrawler 的
first_seen 保留與失敗處理,以及用 fixture 資料完整跑一次 `build_site.main()`
的煙霧測試。CI 在裝完依賴後先跑測試,失敗就不會爬取/部署。
