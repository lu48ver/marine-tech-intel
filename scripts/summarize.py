"""Generate Traditional-Chinese key-point summaries for crawled items.
...
same article is never paid for twice across daily runs.
...
SYSTEM_PROMPT = (
    ...
    "numbers, dates, or figures. If the source text is too short or "
    "irrelevant to summarize, reply with exactly NO_SUMMARY."
)
...
def summarize_text(client, title: str, text: str) -> str:
    """Call the LLM; return Chinese summary, or "" if it declines."""
    ...
    return "" if "NO_SUMMARY" in out else out
...
def main() -> int:
    ...
    calls = 0
    stats = {"cached": 0, "summarized": 0, "skipped": 0, "would_summarize": 0}
    ...
    for path in sorted(glob.glob(str(UPDATES_DIR / "*.json"))):
        ...
        for item in data.get("items", []):
            ...
            if url in cache:  # already summarized in a previous run
                item["summary_zh"] = cache[url]["summary_zh"]
                stats["cached"] += 1
                changed = True
                continue
            ...
            summary_zh = summarize_text(client, item.get("title", ""), text)
            calls += 1
            if not summary_zh:
                stats["skipped"] += 1
                continue
            ...
            stats["summarized"] += 1
            changed = True
            logger.info("summarized (%s) %s", kind, item.get("title", "")[:60])
