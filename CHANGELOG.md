...
## 2026-09-04 (OpenRouter 上線後的調校)

- **壅塞改用「換模型」而非「等待」**:首次實跑 22 篇時整步驟花了 13 分鐘,
  幾乎都在等 429 退避;把間隔拉到 30 秒仍持續被限流,證明瓶頸不是我們的速度,
  而是免費模型的**共用容量**(429 訊息為 "Provider returned error")。
  `chat_json` 改為一輪內依序試完整條模型鏈,整條都忙才退避並放寬間隔。
  實測:選題步驟從約 5 分鐘降到 **32 秒**(GLM 忙 → 立即換 MiniMax 成功)。
- **`BaseCrawler.get` 加重試**(3 次、遞增退避):來源網站對 CI 機房 IP 偶爾
  斷線,一次 TCP 握手失敗不該賠上整天資料。4xx(429 除外)不重試。
- **Paris MoU 解析為空也重試**:該站從 CI 有時回 200 但頁面沒有任何列表列,
  先前會直接記成「頁面結構改變」;改為重新抓取,3 次都空才放棄。
- 測試 +3(重試成功、4xx 不重試、重試上限),共 42 條。
